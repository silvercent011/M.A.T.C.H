    ...
    <SidebarInset>
        ...
        <div class="flex flex-1 flex-col gap-4 p-4 h-[calc(100vh-73px)] overflow-auto">
            <div v-if="job" class="flex flex-col gap-4">
                ...
                <!-- Score banner -->
                <div v-if="job.status === 'done' && job.score_total !== null"
                ...
                <div v-if="job.status === 'processing'"
                    ...
                    <div class="size-10 rounded-full border-4 border-blue-500 border-t-transparent animate-spin" />
                    <p class="font-medium">Agente ATS trabalhando...</p>
                    <p class="text-sm text-muted-foreground">
                        O pipeline está analisando a vaga e otimizando seu currículo. Isso pode levar de 1 a
                        3 minutos.
                    </p>
                </div>

                <!-- Error state -->
                <div v-if="job.status === 'error'"
                ...
                <div v-if="job.status === 'done'" class="flex flex-col gap-4">
                    ...
                    <div class="flex items-center justify-between border-b">
                        <div class="flex gap-1">
                            <button v-for="tab in [
                                ...
                                class="px-4 py-2 text-sm font-medium transition-colors border-b-2 -mb-px" :class="activeTab === tab.key
                        ...
                        <div v-show="activeTab === 'resume'" class="flex gap-2 pb-2">
                            <Button size="sm" variant="outline" @click="downloadMarkdown" :disabled="isExportingPdf">
                    ...
                    <div v-if="activeTab === 'resume'">
                        <pre
                            class="whitespace-pre-wrap font-mono text-sm bg-muted rounded-lg p-4 overflow-auto max-h-[60vh]">{{ job.optimized_resume }}</pre>
                    </div>

                    <!-- Mapa de Requisitos -->
                    <div v-if="activeTab === 'requirements'">
                        <pre
                            class="whitespace-pre-wrap text-sm bg-muted rounded-lg p-4 overflow-auto max-h-[60vh]">{{ job.job_requirements_map }}</pre>
                    </div>

                    <!-- Auditoria -->
                    <div v-if="activeTab === 'audit'">
                        <pre
                            class="whitespace-pre-wrap text-sm bg-muted rounded-lg p-4 overflow-auto max-h-[60vh]">{{ job.audit_report }}</pre>
                    </div>
                </div>
                ...
                <div class="rounded-lg border bg-card p-4">
                    ...
                    <pre
                        class="whitespace-pre-wrap text-sm text-muted-foreground max-h-[30vh] overflow-auto">{{ job.job_description }}</pre>
                </div>
            </div>
        </div>
    </SidebarInset>
