    ...
    <Sidebar collapsible="none" class="hidden flex-1 md:flex md:flex-col w-full">
        <SidebarHeader class="gap-3.5 border-b p-4">
            <div class="flex w-full items-center justify-between">
                <div class="text-base font-medium text-foreground flex items-center gap-2">
        ...
        <SidebarContent>
            <SidebarGroup class="px-0">
                <SidebarGroupContent>
                    ...
                    <div v-if="filteredJobs.length === 0" class="p-4 text-center text-sm text-muted-foreground">
                        Nenhuma vaga encontrada.
                    </div>

                    <button v-for="job in filteredJobs" :key="job.id" @click="selectJob(job)"
                        ...
                        :class="activeJob?.id === job.id ? 'bg-sidebar-accent text-sidebar-accent-foreground' : ''">
                        ...
                        <div class="flex w-full items-center gap-2">
                            <span class="line-clamp-1 w-[200px] whitespace-break-spaces text-xs text-muted-foreground">
                                {{ job.job_description }}
                            </span>
            ...
            <form @submit.prevent="createJob">
                <div class="grid gap-4 py-4">
                    <div class="grid gap-2">
                        ...
                        <Input id="job-title" v-model="createForm.job_title" required
                            placeholder="Ex: Engenheiro de Software Sênior — TechCorp" />
                    </div>
                    <div class="grid gap-2">
                        <Label htmlFor="base-resume">Currículo Base</Label>
                        <select id="base-resume" v-model="createForm.base_resume_id" required
                    ...
                    <div class="grid gap-2">
                        ...
                        <Textarea id="job-description" v-model="createForm.job_description" required rows="12"
                            class="font-mono text-sm resize-none"
                            placeholder="Cole aqui a descrição completa da vaga..." />
                    </div>
                </div>
                <DialogFooter>
                    <Button type="button" variant="outline" @click="openCreateDialog = false">Cancelar</Button>
